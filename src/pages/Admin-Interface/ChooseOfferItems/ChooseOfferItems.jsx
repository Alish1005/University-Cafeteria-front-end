...
function ChooseOfferItems(props) {
  ...
  const refresh = () => {
    //Save Sections
    axios
      .get(variables.API_URL + "Item/Sections")
      .then((res) => {
  ...
  return (
    <div className="pagesContent">
      ...
      <div className="row">
                ...
                <TabPanel value={`${section.id}`}>
                  {items
                    ...
                    .map((s) => (
                      <button
                        onClick={() => addItem(s)}
                        className="col-3 m-2 text-black btn btn-secondary"
                      >
                        <p>{s.name}</p>
                        <p>{s.price}$</p>
                      </button>
        ...
        {Offer.id == 0 || Offer.id == null ? (
          <Link
            to="/OfferList/AddOffer"
            className="btn-makeOrder btn btn-primary text-secondary"
            onClick={() => {
              setOfferItems(orderlist);
              setOfferTotal(total);
            }}
          >
            Submit items
            ...
            <div class="dropdown">
              ...
              <ul class={`dropdown-menu bg-gray`}>
                {total > 0 && (
                  <li>
                    <a
                      class="dropdown-item rounded-bottom"
                      onClick={() => window.print("OrderListStaff")}
                      href="#"
                    >
                      <PrintIcon /> print
                    </a>
